...
vec aes_crypt_msg(EVP_CIPHER_CTX *ctx, const unsigned char *start, int count) {
  //figure out the block size that AES is going to use.
  int cipher_block_size = EVP_CIPHER_block_size(EVP_CIPHER_CTX_cipher(ctx));
  //Set up buffer where AES puts crypted bits
  
  std::vector<unsigned char> outbuf;
  outbuf.reserve(AES_BLOCKSIZE + cipher_block_size);
  ...
  printf("array:%s\nvector:",out_buf);
  for(long unsigned int i = 0; i < sizeof(out_buf);i++){
	  if(out_buf[i]=='\0'){
		  break;
	  }
	  outbuf.push_back(out_buf[i]);
	  printf("%c",out_buf[i]);
  }
  printf("\n");
  ...
  printf("\nReal Vector: ");
  for(long unsigned int i = 0; i < outbuf.size(); i++){
	  printf("%c",outbuf.at(i));
  }
  
  cerr << "aes_crypt_msg is not implemented\n";
  return outbuf;
}
...
vec create_aes_key() {
  vec key(AES_KEYSIZE + AES_IVSIZE);
...
int main(int argc, char**argv){
	/*std::vector key = create_aes_key();
	EVP_CIPHER_CTX *evp = create_aes_context(key, true);
