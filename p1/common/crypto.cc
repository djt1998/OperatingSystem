...
vec aes_crypt_msg(EVP_CIPHER_CTX *ctx, const unsigned char *start, int count) {
  ...
  vec out_buf(AES_BLOCKSIZE);
  //Set up buffer where AES puts crypted bits
  
  int outlen;
  int enclen = 0;
  
  while(count>AES_BLOCKSIZE){
	if (!EVP_CipherUpdate(ctx, out_buf.data()+enclen, &outlen, start, AES_BLOCKSIZE)) 
	{
	...
	if (!EVP_CipherUpdate(ctx, out_buf.data()+enclen, &outlen, start, count)) 
	{
		fprintf(stderr, "Error in EVP_CipherUpdate: %s\n",
		ERR_error_string(ERR_get_error(), nullptr));
		return out_buf;
  ...
  if (!EVP_CipherFinal_ex(ctx, out_buf.data()+enclen, &outlen)) {
    fprintf(stderr, "Error in EVP_CipherFinal_ex: %s\n",
            ERR_error_string(ERR_get_error(), nullptr));
    return out_buf;
  }
  enclen += outlen;
  out_buf.resize(enclen);
  return out_buf;
}
